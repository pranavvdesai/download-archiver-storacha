...
(async () => {
  try {
    ...
    if (!email) {
      ...
      accountLabel.className = "status-label";
      accountLabel.textContent = "Account";

      const accountValue = document.createElement("div");
      accountValue.className = "status-value";
      accountValue.textContent = email;

      status.appendChild(accountLabel);
      status.appendChild(accountValue);

      if (spaceDid) {
        const spaceLabel = document.createElement("div");
        spaceLabel.className = "status-label";
        spaceLabel.style.marginTop = "12px";
        spaceLabel.textContent = "Space";

        const spaceValue = document.createElement("div");
        spaceValue.className = "status-value";
        spaceValue.textContent = spaceDid;

        status.appendChild(spaceLabel);
        status.appendChild(spaceValue);
      } else {
        const spaceLabel = document.createElement("div");
        spaceLabel.className = "status-label";
        spaceLabel.style.marginTop = "12px";
        ...
        const spaceValue = document.createElement("div");
        spaceValue.className = "status-value";
        spaceValue.style.color = "#868e96";
        spaceValue.textContent = "Pending configuration...";
    ...
    const status = document.getElementById("status");
    status.innerHTML = `<div class="not-configured">Error loading status.</div>`;
  }
})();
